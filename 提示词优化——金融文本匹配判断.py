...
for answer,sentences in examples_data.items():
    for sentence in sentences:
        messages.append({'role':'user','content':f'句子1： [{sentence[0]}]句子2：[{sentence[1]}]'})
        messages.append({'role':'assistant','content':answer})

for question in questions:
    response=client.chat.completions.create(
        model='glm-5.3-flash',
        messages=messages+[{'role':'user','content':f"按照上述示例，回答这2个句子的情况。句子1: [{question[0]}]，句子2: [{question[1]}]"}]
    )
